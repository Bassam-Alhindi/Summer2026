...
class AssistantContextTest extends TestCase
{
    ...
    public function test_prompt_carries_the_strict_grounding_rules(): void
    {
        ...
        // Loosening the tone must not loosen the grounding: every input still comes
        // from the snapshot, a tool result, or the user's own message.
        $this->assertStringContainsString('never from memory or guesswork', $prompt);
        $this->assertStringContainsString('you never invent a number', $prompt);
        $this->assertStringContainsString('THE ONLY SOURCE OF TRUTH', $prompt);
        $this->assertStringContainsString('Never invent an input', $prompt);
        $this->assertStringContainsString(
            'Ask for the missing figure instead of assuming one',
            $prompt,
        );
    }
    ...
    public function test_prompt_asks_for_warm_but_answer_first_replies(): void
    {
        $user = User::factory()->create();
        $this->seedLedger($user);
        ...
        $this->assertStringContainsString('WARM, DIRECT, HELPFUL', $prompt);
        $this->assertStringContainsString('ANSWER FIRST.', $prompt);
        $this->assertStringContainsString('never after a caveat', $prompt);
        $this->assertStringContainsString('No rigid refusals', $prompt);
    }

    public function test_prompt_permits_hypothetical_budgeting_arithmetic(): void
    {
        $user = User::factory()->create();
        $this->seedLedger($user);
        ...
        $this->assertStringContainsString('THIS IS ARITHMETIC, NOT PREDICTION', $prompt);
        $this->assertStringContainsString(
            'Subtract expenses the user names from a balance',
            $prompt,
        );
        $this->assertStringContainsString('You may freely do arithmetic', $prompt);

        // The old blanket bans would have vetoed the whole budgeting feature.
        $this->assertStringNotContainsString('Never estimate, approximate, extrapolate', $prompt);
        $this->assertStringNotContainsString('NEVER estimate, calculate', $prompt);
    }
